...
(function (mtr) {
    ...
    _this.item_exists = function (haystack, needle) {
        for (var i = 0; i < haystack.length; i++) {
            if (_this.compare_objects(haystack[i], needle)) {
                return true;
            }
        }
        return false;
    ...
    _this.search_objects = function (query, objects) {
        ...
        for (var i = 0; i < objects.length; i++) {
            for (var key in objects[i]) {
                if (objects[i].hasOwnProperty(key) && _this.is_string(objects[i][key])) {
                    var obj = objects[i][key].toLowerCase();
                    if (obj.indexOf(query) !== -1) {
                        if (!_this.item_exists(results, objects[i])) {
